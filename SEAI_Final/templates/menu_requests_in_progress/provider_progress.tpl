...
{foreach $requests_new as $request_n}
        <tr>
        <td><form id="{$request_n.request_id}_re" method="post" action="{$BASE_URL}actions/open_req_pag.php">
...
{foreach $requests_old as $request_o}
	{$result=getSensorTypesResolutionValues($request_o.mission_id)}
        <tr>
        <td><form id="{$request_o.request_id}_req" method="post" action="{$BASE_URL}actions/open_req_pag.php">
                  <input type="hidden" name="request_id" value="{$request_o.request_id}">
				  <a href="#" onclick="document.getElementById('{$request_o.request_id}_req').submit()" style="color:white;">{$request_o.request_id}</a>
        </form>
		</td>
		<td>{$result.sensor_type}</td>
		<td>{$result.resolution_type}</td>
    <td>
      <form id="{$request_o.request_id}_{$request_o.client_username}_seePersonnalPage_OldData" method="post" action="{$BASE_URL}pages/user_page.php">
        <input type="hidden" name="userinfo_username" value="{$request_o.client_username}">
        <a href="#" onclick="document.getElementById('{$request_o.request_id}_{$request_o.client_username}_seePersonnalPage_OldData').submit()" style="color:white;">{$request_o.client_name}</a>
      </form>
    </td>

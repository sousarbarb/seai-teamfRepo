...
<div class="menusLogin p-5">
    ...
    {$requests = getAllRequests( $user_id )}



    {foreach $requests as $request}
        <tr>
        <td>{$request['request_id']}</td><td>{$request['client_name']}</td><td>{$request['request_sensor_type']}</td><td>{$request['request_res_value']}</td><td>{$request['polygon']}</td><td>{$request['request_comments']}</td>
